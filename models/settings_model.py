...
class SettingsModel:
    ...
    def _ensure_settings_file(self) -> None:
        ...
        if not os.path.exists(self.settings_file):
            default_settings = [
                ...
                # Response timeout
                ["response_timeout", "10", "True"],
                # Max verification attempts
                ["max_verification_attempts", "3", "True"],
                # Browser sequences for different providers
                ["google_browser_sequence", "edge,chrome,chrome_normal", "True"],
                ["microsoft_browser_sequence", "edge,chrome,edge_normal,firefox", "True"],
                ["default_browser_sequence", "edge,chrome,firefox", "True"]
            ]
    ...
    def load_settings(self) -> None:
        ...
        try:
            ...
            self.settings = {
                ...
                "response_timeout": {"value": "10", "enabled": True},
                "max_verification_attempts": {"value": "3", "enabled": True},
                "google_browser_sequence": {"value": "edge,chrome,chrome_normal", "enabled": True},
                "microsoft_browser_sequence": {"value": "edge,chrome,edge_normal,firefox", "enabled": True},
                "default_browser_sequence": {"value": "edge,chrome,firefox", "enabled": True}
            }
    ...
    def configure_browser_settings(self) -> None:
        ...
        current_max_attempts = self.get("max_verification_attempts", "3")
        current_response_timeout = self.get("response_timeout", "10")
        current_google_sequence = self.get("google_browser_sequence", "edge,chrome,chrome_normal")
        current_microsoft_sequence = self.get("microsoft_browser_sequence", "edge,chrome,edge_normal,firefox")
        current_default_sequence = self.get("default_browser_sequence", "edge,chrome,firefox")
        ...
        print(f"Max verification attempts: {current_max_attempts}")
        print(f"Response timeout: {current_response_timeout} seconds")
        print(f"Google browser sequence: {current_google_sequence}")
        print(f"Microsoft browser sequence: {current_microsoft_sequence}")
        print(f"Default browser sequence: {current_default_sequence}")
        ...
        max_attempts = input("Enter maximum verification attempts per email: ")
        try:
            max_attempts = max(1, int(max_attempts))
            self.set("max_verification_attempts", str(max_attempts), True)
        except ValueError:
            pass
        
        response_timeout = input("Enter response timeout in seconds: ")
        try:
            response_timeout = max(1, int(response_timeout))
            self.set("response_timeout", str(response_timeout), True)
        except ValueError:
            pass
        
        google_sequence = input("Enter browser sequence for Google accounts (comma-separated): ")
        if google_sequence:
            self.set("google_browser_sequence", google_sequence, True)
        
        microsoft_sequence = input("Enter browser sequence for Microsoft accounts (comma-separated): ")
        if microsoft_sequence:
            self.set("microsoft_browser_sequence", microsoft_sequence, True)
        
        default_sequence = input("Enter default browser sequence for other providers (comma-separated): ")
        if default_sequence:
            self.set("default_browser_sequence", default_sequence, True)
    ...
    def configure_selenium_settings(self) -> None:
        """Configure Selenium behavior settings."""
        print("\nSelenium Behavior Settings:")
        current_human_behavior = self.is_enabled("human_behavior_enabled")
        current_input_validation = self.is_enabled("input_validation_enabled")
        current_response_timeout = self.get("response_timeout", "10")
        
        print(f"Human behavior is currently {'enabled' if current_human_behavior else 'disabled'}")
        print(f"Input validation is currently {'enabled' if current_input_validation else 'disabled'}")
        print(f"Response timeout: {current_response_timeout} seconds")
        
        human_behavior = input("\nEnable human behavior (realistic typing and delays)? (y/n): ")
        if human_behavior.lower() == 'y':
            self.set("human_behavior_enabled", "True", True)
            print("\nHuman behavior enabled")
        else:
            self.set("human_behavior_enabled", "False", False)
            print("\nHuman behavior disabled")
        ...
        try:
            response_timeout = max(1, int(response_timeout))
            self.set("response_timeout", str(response_timeout), True)
            print(f"\nResponse timeout set to {response_timeout} seconds")
        except ValueError:
            pass
